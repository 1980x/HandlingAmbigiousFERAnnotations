    ...
    def train(self, train_loader, epoch):
        ...
        for i, (images1, images2, labels, indexes) in enumerate(train_loader):
            ...
            with torch.no_grad():
                ...
                js_all = js_div(probs1, given_labels)
                js = js_all.mean()
            ...
            #loss_c = losses[prob_clean >= threshold_clean].mean()
            loss_c = self.ce_loss(logits1,labels) + self.ce_loss(logits2, labels)
            # consistency loss
        ...
        print('%.4f,%.4f,%.4f,%.4f'%(loss_c,loss_o,loss, js))
